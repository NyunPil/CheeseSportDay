...
// -------------------------------------------------------------
//  버튼 하나로 순서대로 전부 생성:
//    ① 아레나(드래프트룸)  ② 갤러리홀 + 소품  ③ 텍스처 + 창문  ④ 액자
//  (포탈은 텔레포트 연결이 수동이라 제외 — 포탈 배치기로 따로)
//
    ...
    public static class OneClickBuild
    {
        ...
        static void Run()
        {
            bool ok = EditorUtility.DisplayDialog(
                "⭐ 원클릭 전체 생성",
                "아레나(드래프트룸) → 갤러리홀/소품 → 텍스처/창문 → 액자 순서로 새로 생성합니다.\n\n" +
            ...
            try
            {
                ...
                EditorUtility.DisplayProgressBar("원클릭 전체 생성", "④ 액자 생성…", 0.85f);
                FramePlacer.BuildDefault();

                EditorSceneManager.MarkSceneDirty(EditorSceneManager.GetActiveScene());
                Debug.Log("⭐ 원클릭 전체 생성 완료! (아레나 → 갤러리홀/소품 → 텍스처/창문 → 액자)");
            }
